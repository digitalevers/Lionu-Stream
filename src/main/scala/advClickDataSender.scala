 ...
 * 广告数据发生器
 */
object advClickDataSender {
  def main(args: Array[String]): Unit = {
    //读取配置
    val prop = new Properties()
    val inputStream = getClass.getClassLoader.getResourceAsStream("application.properties")
    prop.load(inputStream);
    val phpApiUrl = prop.get("phpApiUrl")
    val proxy_open = prop.get("proxy.open")


    val host = phpApiUrl+"/receive/clickhouse"
    //println(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date))

    val androidClickData = constructAndroidClickData
    ...
    println(end)*/
      ...
      //设置请求配置
      val proxyHost = new HttpHost(prop.get("proxy.host").toString, prop.get("proxy.port").toString.toInt)
      var requestConfig:RequestConfig = null
      if(proxy_open == "1") {
        requestConfig = RequestConfig.custom().setConnectTimeout(10000).setSocketTimeout(10000).setProxy(proxyHost).build() //设置代理
      } else {
        requestConfig = RequestConfig.custom().setConnectTimeout(10000).setSocketTimeout(10000).build() //设置代理
      }
    ...
    val deviceApi = phpApiUrl+"/receive/launch"

    val request: HttpRequest = Http(deviceApi)
    if(proxy_open == "1"){
      request.postForm(androidDeviceData.toSeq).proxy(prop.get("proxy.host").toString, prop.get("proxy.port").toString.toInt).asString
    } else {
      request.postForm(androidDeviceData.toSeq).asString
    }
    //println(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date))
  }
  ...
  def constructAndroidClickData(): mutable.Map[String, String] = {
    ...
    map += ("imei" -> "eae5ab42f65c8553e40419948800ef3d")
    map += ("mac" -> simulateMAC)
    ...
    map += ("ts" -> getMillTimes)
    map += ("os" -> "0")
    val appid = (Random.nextInt(10) + 1).toString
    val plan_id = (Random.nextInt(10)+1).toString
    val channel_id = (Random.nextInt(10)+1).toString
    map += ("appid" -> "5")
    map += ("plan_id" -> plan_id)
